function  [numgrad,mangrad] = f_M(varargin)
...
% to compare these, if equal, then porve derivation formula is right
    ...
    if nargin >= 3
        ...
        theta = [0.4898,0.7547,0.1626,0.3404;
    ...
    for i=1:length(theta)
        for j = 1:length(theta)
            d=zeros(size(theta));
            d(i,j) = EPSILON;
            numgrad(i,j)=(J(theta+d/2,D,lambda)-J(theta-d/2,D,lambda))/EPSILON;
        end
    end
    
    % show the manual gradient, same as hand write
    mangrad = manual_gradient(theta,D,lambda);
end

function res = m_function(theta,D,lambda)
...
function mangrad = manual_gradient(theta,D,lambda)
% Initialize numgrad with zeros
    M = theta;
    ...
    M = (M + lambda*(D^2));
    size_of_M = size(M);
    mangrad = M\ones(size_of_M(1),1);
    mangrad = mangrad*mangrad_f_c_tilde';
    mangrad = mangrad/M;
    mangrad = -mangrad';
end
